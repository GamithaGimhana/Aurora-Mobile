...
export default function Home() {
  ...
  const { notes, loading: notesLoading } = useAppSelector((state) => state.notes);
  const { cards, loading: cardsLoading } = useAppSelector((state) => state.flashcards);

  const isLoading = notesLoading || cardsLoading;
      ...
      <SafeAreaView className="flex-1">
        ...
        >
          ...
          <View className="px-8 pt-8 pb-6">
            <Animated.Text entering={FadeInUp.delay(100)} className="text-gray-400 text-xs font-bold uppercase tracking-[2px]">
              Master Everything
            </Animated.Text>
            <Animated.Text entering={FadeInUp.delay(200)} className={`${primaryText} text-4xl font-black tracking-tighter mt-1`}>
              ...
              >
                <View className="flex-row justify-between items-center mb-6">
                  <View className={`${darkMode ? "bg-white/20" : "bg-purple-100"} px-4 py-2 rounded-full flex-row items-center`}>
                    ...
                    <Text className={`${darkMode ? "text-white" : "text-purple-700"} text-[10px] font-bold ml-2 uppercase tracking-widest`}>Growth</Text>
                  </View>
                ...
                <View className="flex-row items-end justify-between">
                  <View>
                    <Text className={`${darkMode ? "text-white" : "text-[#1A1A1A]"} text-4xl font-black`}>{notes.length + cards.length}</Text>
                    <Text className={`${darkMode ? "text-white/60" : "text-gray-400"} text-xs font-bold uppercase mt-1`}>Study Assets</Text>
                  </View>
                  <View className="items-end">
                    <Text className={`${darkMode ? "text-white" : "text-purple-600"} text-lg font-bold`}>Keep Going</Text>
                    <Text className={`${darkMode ? "text-white/50" : "text-gray-400"} text-[10px]`}>Ready for session</Text>
                  </View>
                </View>
              </Animated.View>
            )}
            ...
            <View className="flex-row gap-x-4">
              <Animated.View entering={FadeInDown.delay(400)} className="flex-1">
                <Pressable 
                  ...
                  className={`${cardBg} border ${cardBorder} p-6 rounded-[35px] h-44 justify-between shadow-sm active:scale-95`}
                >
            ...
            <Animated.View entering={FadeInDown.delay(600)} className="mt-6">
              ...
              {isLoading ? (
                ...
                notes.slice(0, 3).map((note, index) => (
                  <Animated.View key={note.id} entering={FadeInDown.delay(index * 100)}>
                    <Pressable 
                      onPress={() => router.push({ pathname: "/(dashboard)/notes/view", params: { noteId: note.id } })}
                      className={`${cardBg} border ${cardBorder} p-5 rounded-[28px] flex-row items-center mb-3 shadow-sm active:bg-purple-500/5`}
                    >
                      ...
                      <ArrowUpRight size={16} color={darkMode ? "#4B5563" : "#D1D5DB"} />
                    </Pressable>
                  </Animated.View>
                ))
              )}
