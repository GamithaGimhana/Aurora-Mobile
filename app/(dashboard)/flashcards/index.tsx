...
import Animated, { FadeInDown, FadeInUp, useAnimatedStyle, withTiming } from "react-native-reanimated";

export default function FlashcardsIndex() {
  const router = useRouter();
  const dispatch = useAppDispatch();
  
  // Section Toggle State: 'sets' or 'cards'
    ...
    <View className="flex-1 bg-[#050505]">
      ...
      <SafeAreaView className="flex-1">
        ...
        <View className="px-6 flex-row items-center justify-between py-6">
          <View>
            <Text className="text-white text-3xl font-black tracking-tighter">Aurora Flash</Text>
        ...
        <View className="px-6 mb-6">
          <View className="bg-white/5 p-1 rounded-2xl flex-row border border-white/10">
            <Pressable 
            ...
            >
              <Layers size={16} color={activeSection === 'sets' ? "white" : "#6B7280"} />
              <Text className={`ml-2 font-bold ${activeSection === 'sets' ? 'text-white' : 'text-gray-500'}`}>Sets</Text>
            </Pressable>
            ...
            >
              <FileText size={16} color={activeSection === 'cards' ? "white" : "#6B7280"} />
              <Text className={`ml-2 font-bold ${activeSection === 'cards' ? 'text-white' : 'text-gray-500'}`}>All Cards</Text>
            </Pressable>
          </View>
        </View>
        ...
        >
          {loading && cards.length === 0 ? (
            ...
            <Animated.View entering={FadeInUp} className="py-20 items-center justify-center">
              ...
              <Text className="text-white text-xl font-bold">No Data Yet</Text>
              <Text className="text-gray-500 text-center mt-2">Create your first flashcard to see it here.</Text>
            </Animated.View>
          ) : (
            <>
              {/* RENDER STUDY SETS SECTION */}
              ...
              {/* RENDER INDIVIDUAL CARDS SECTION */}
